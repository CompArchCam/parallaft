...
use parallaft::comparators::memory::MemoryComparatorType;
use parallaft::dirty_page_trackers::DirtyPageAddressTrackerType;
use parallaft::helpers::cpufreq::{CpuFreqGovernor, CpuFreqScalerType};
use parallaft::slicers::{ReferenceType, SlicerType};
use parallaft::statistics::hwmon::HwmonSensorPath;
use parallaft::types::perf_counter::symbolic_events::BranchType;
use parallaft::StatsOutput;
use parallaft::{statistics::perf::CounterKind, RelShellOptions};
...
fn main() {
    ...
    let exit_status = parallaft::run(Command::new(cli.command).args(cli.args), config);
